...
/**
 * Adaptador por defecto del {@link IndicadorInventarioAvanzadoPort} que devuelve
 * indicadores vacios (Req 22.1, 22.2). Se registra solo si no existe otra
 * implementacion ({@code @ConditionalOnMissingBean}). Solo lectura, sin acceso a datos
 * de origen.
 */
public class IndicadorInventarioAvanzadoVacio implements IndicadorInventarioAvanzadoPort {

    @Override
    public IndicadoresArea agregar(FiltroIndicadores filtro) {
        return IndicadoresArea.vacio(AreaIndicador.INVENTARIO_AVANZADO);
    }
}
