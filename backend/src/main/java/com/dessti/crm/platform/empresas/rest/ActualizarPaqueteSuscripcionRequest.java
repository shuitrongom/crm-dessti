...
/**
 * Cuerpo de la peticion para actualizar un Paquete de Suscripcion (Req 3.1).
 * Rediseno {@code plan-vs-suscripcion-contratacion}: es el <strong>espejo</strong>
 * de {@link ActualizarPlanRequest} para el catalogo de <strong>contratos de
 * corto plazo</strong> (duracion de un año o menos) con opcion de periodo de
 * prueba. Un Paquete pertenece a un Giro, se cotiza en una moneda y lleva un
 * precio por modulo; las claves de los modulos habilitados se derivan de las
 * claves de {@code preciosModulos} (no hay un campo separado).
 *
 * @param nombre              nuevo nombre del Paquete (unico); obligatorio.
 * @param maxUsuarios         nuevo maximo de Usuarios; {@code >= 0}.
 * @param duracionDias        nueva duracion del contrato en dias; {@code 1..365}
 ...
 *                            (3 letras).
 * @param preciosModulos      nuevo mapa {@code clave -> precio}; obligatorio
 *                            (puede ir vacio). Se normaliza y valida en el
 *                            dominio.
 */
public record ActualizarPaqueteSuscripcionRequest(
